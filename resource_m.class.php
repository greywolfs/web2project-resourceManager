...
function getStartOfWeek($date) {
	$cdate=new w2p_Utilities_Date($date);
	if($cdate->format('%w') == 0)
		return $date;
	else {
		$cdate->addDays(-1);
...
function loadProjectsOf($tasks) {
	...
	$q->addWhere('project_status=0');
	$q->addQuery('DISTINCT(project_id)');
	$where = '(';
	foreach ($tasks as $task) {
		$where .= 'project_id = '.$task->task_project. ' OR ';
	}
	if ($where!='(') {
...
function editDur($user_id, $task_id, $new_sdate, $new_edate, $adapt=false) {
	global $dPconfig;
	$cnew_sdate = new w2p_Utilities_Date($new_sdate);
	$cnew_edate = new w2p_Utilities_Date($new_edate);
	...
	if ($adapt) { // If user request an adaptation
		...
		$q->addWhere('task_id = '.$task_id);
		$cold_sdate = new w2p_Utilities_Date($q->loadResult());
		$q = new w2p_Database_Query;
		$q->addTable('tasks', 't');
		$q->addQuery('task_end_date');
		$q->addWhere('task_id = '.$task_id);
		$cold_edate = new w2p_Utilities_Date($q->loadResult());
...
function editAssign($user_id, $task_id, $new_percent, $aff_style, $adapt=false) {
	...
	if ($adapt) { // If user request an adaptation
		...
		$q->addQuery('task_start_date');
		$q->addWhere('task_id = '.$task_id);
		$csdate = new w2p_Utilities_Date($q->loadResult());
		$q = new w2p_Database_Query;
		$q->addTable('tasks', 't');
		$q->addQuery('task_end_date');
		$q->addWhere('task_id = '.$task_id);
		$cedate = new w2p_Utilities_Date($q->loadResult());
		...
		$nedate = new w2p_Utilities_Date($csdate);
...
function getAssignment($ccurrent, $task, $user_id, $scale, $show) {
	if($show < 3) { // The assignment should be calculate for a ranged date so we need to average.
		...
		$ctmp 	= new w2p_Utilities_Date($ccurrent);
		$cnext 	= new w2p_Utilities_Date($ccurrent);
		nextDate($cnext, $show);
		$show	= 3; 
		prevDate($cnext, $show);
		prevDate($ctmp, $show);
