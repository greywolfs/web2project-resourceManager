...
$cinOneMonth = new w2p_Utilities_Date();
$cinOneMonth->addMonths(1);
$inOneMonth = $cinOneMonth->format(FMT_TIMESTAMP_DATE);

// Get the param
$project_id = w2PgetParam($_POST, 'project_id', 0); 								// Project filter 					Default: all(0)
$company 	= w2PgetParam($_POST, 'company_id', 0);								// Company/Department filter		Default: all(0)
$contact_id = w2PgetParam($_POST, 'contact_id', getContactId($AppUI->user_id));	// Contact filter					Default: user
$aff_style 	= w2PgetParam($_POST, 'aff_style', 1);								// 0 : % , 1 : daily assigned hours Default: daily assigned hours
$dyna 		= w2PgetParam($_POST, 'dyna', 0);									// 0 : show dynamic tasks, 1 : hide	Default: show dynamic tasks
$start_date = w2PgetParam($_POST, 'res_start_date', $today);							// Start Date filter 				Default: today
$end_date 	= w2PgetParam($_POST, 'res_end_date', $inOneMonth); 						// End Date filter					Default: today + 1 month
$toExpand	= w2PgetParam($_POST, 'expandedList', null);							// List of <tr> to re-expand		Default: none(null)
...
if ($company_id == '') {
	$company_id 	= 	0;
	$department_id 	= 	w2PgetParam($_POST, 'company_id', 0);
	$department		= 	''.$department_id;
} else
	$department_id 	= 	0;

// Count how many days, weeks, months, year are in range date
$cstart_date	= 	new w2p_Utilities_Date($start_date);
$cend_date 		= 	new w2p_Utilities_Date($end_date);
...
// Calculate the scale
$scale = ($aff_style) ? w2PgetConfig('daily_working_hours') : 100;

// Calculate the number of columns needed
$colCount = ($showHours) ? w2PgetConfig('daily_working_hours') : (($showDays) ? $dayCount : (($showWeeks) ? $weekCount : (($showMonths) ? $monthCount : $yearCount)));
...
if (isset($_POST['aTId']) && $_POST['aTId'] != "..."){
	if (getPermission('tasks', 'edit', $_POST['aTId'])) {
		switch ($_POST['editType']) {
			...
			case 'aa' : editDur($_POST['aUId'],$_POST['aTId'],$_POST['res_aSDate'],$_POST['res_aEDate'],$_POST['aAdaptA']);break;
			default : break;
		}
	} else
		$AppUI->redirect("m=public&a=access_denied");
	...
	<table class="tbl std" cellspacing="0" cellpadding="4" border="0" width ="100%">
		<tbody>
			<tr>
				...
				$psdate = new w2p_Utilities_Date($start_date); $psdate->addMonths(-1);
				$pedate = new w2p_Utilities_Date($end_date);	$pedate->addMonths(-1);
				$nsdate = new w2p_Utilities_Date($start_date); $nsdate->addMonths(1);
				...
				<td align="left" nowrap="nowrap">
					<input type="hidden" id="res_start_date" name="res_start_date" value="<?php echo $start_date; ?>" />
					<input type="text" class="text" id="start_date" name="start_date" value="<?php echo $cstart_date->format("%d.%m.%Y"); ?>" size="12" disabled="disabled" onchange="setDate_new('mainFrm', 'start_date');" />
					<a href="javascript: void(0);" onclick="return showCalendar('start_date', '<?php echo $df ?>', 'mainFrm', null, true, true)">
						<img src="<?php echo w2PfindImage('calendar.gif'); ?>" width="24" height="12" alt="<?php echo $AppUI->_('Calendar'); ?>" border="0" />
					</a>
				</td>
				<td align="right" nowrap="nowrap"><?php echo $AppUI->_('To').':';?></td>
				<td align="left" nowrap="nowrap">
					<input type="hidden" id="res_end_date" name="res_end_date" value="<?php echo $end_date; ?>" />
					<input type="text" class="text" id="end_date" name="end_date" value="<?php echo $cend_date->format("%d.%m.%Y"); ?>" size="12" disabled="disabled" onchange="setDate_new('mainFrm', 'end_date');" />
					<a href="javascript: void(0);" onclick="return showCalendar('end_date', '<?php echo $df ?>', 'mainFrm', null, true, true)">
						<img src="<?php echo w2PfindImage('calendar.gif'); ?>" width="24" height="12" alt="<?php echo $AppUI->_('Calendar'); ?>" border="0" />
					</a>
					...
					foreach ($rows as $row) {
						if ($row['dept_parent'] == 0) {
							if ($comp != $row['company_id']) {
								$cBuffer .= ('<option value="' . $AppUI->__($company_prefix . $row['company_id'])
											 . '" style="font-weight:bold;"' 
											 . (($company.'' == $AppUI->__($company_prefix . $row['company_id'])) ? 'selected="selected"' : '')
											 . '>' . $AppUI->__($row['company_name']) . '</option>' . "\n");
								$comp = $row['company_id'];
							}
...
<!-- Main table -->
<table class="treeTable std" cellspacing="0" cellpadding="0" border="0">
	<thead>
		<tr>
			<th class="thHead">
			...
			<th class="thCal" colspan="<?php echo $colCount; ?>">
				<table class="treeCal" cellspacing="0" cellpadding="0" border="0">
					<tbody>
						...
						echo '<tr>';
						$ccurrent 		= new w2p_Utilities_Date($start_date);
						$colspan 		= 0;
							...
							for($i = 0; $i < $colCount; $i++) {
								if($currentMonth == $ccurrent->getMonthName(true))
									...
									echo '<td class="treeMonth" colspan='.$colspan.'>'
										.generateRangeLink($ccurrent->format('%Y%m').'01', $ccurrent->format('%Y%m%t'),
	...
	foreach ($users as $user) { // Display a line by user
		if(getPermission('contacts', 'view', $user->contact_id)) {
			...
			$user_company=$user->getCompanyDetails();
			$fun 	= 'displayUserDetails(event,\''.addslashes($user_company['company_name']).'\',\''.addslashes($dep['dept_name']).'\');';
			...
			foreach ($projects as $project) {
				if(getPermission('projects', 'view', $project->project_id)) {
					$sdate 		= new w2p_Utilities_Date($project->project_start_date);
					$edate 		= new w2p_Utilities_Date($project->project_end_date);
					$owner 		= new CContact();
						...
						if ($task->task_project == $project->project_id  && getPermission('tasks', 'view', $task->task_id)) {
							if ($task->task_project == $project->project_id) {
								...
								if($parent != null) {
									...
									$sdate 		= new w2p_Utilities_Date($task->task_start_date);
									$edate 		= new w2p_Utilities_Date($task->task_end_date);
									$owner 		= new CContact();
...
<div id="assignEdit" style="position:absolute;top:0px;z-index:0;display:none;">
	...
	<tr>
		...
		<td class="hilite" align="right" valign="top" nowrap>
			<input type="hidden" id="res_aSDate" name="res_aSDate"/>
			<input type="text" class="text" id="aSDate" name="aSDate" size="12" disabled="disabled" onchange="setDate_new('mainFrm', 'aSDate');" />
<!--			<input type="hidden" id="aSDate" name="aSDate" value="" onChange="document.getElementById('aPercent').disabled='disabled';
		...
		<td class="hilite" align="left" valign="top" id="aSCal" style="display:table-cell;" nowrap>
			<a href="javascript: void(0);" onclick="switchTypeAA(); return showCalendar('aSDate', '<?php echo $df ?>', 'mainFrm', null, true, true)"
			    onFocus="if (document.getElementById('res_aSDate').value == start) switchTypeNull();"
				>
				<img src="<?php echo w2PfindImage('calendar.gif'); ?>" width="24" height="12" alt="<?php echo $AppUI->_('Calendar'); ?>" border="0" />
			</a>
<!--			<a href="javascript:popCalendar(document.mainFrm.aSDate)" onClick="switchTypeAA();" onFocus="if (document.getElementById('aSDate').value == start) switchTypeNull();" >
				<img src="<?php /*echo w2PfindImage('calendar.gif'); */?>" width="24" height="12" alt="" border="0">
			</a>-->
		</td>
	</tr>
	<tr>
		<td align="right" class="hilite" valign="top" nowrap><?php echo $AppUI->_("End date").':'; ?></td>
		<td class="hilite" align="right" valign="top" nowrap>
			<input type="hidden" id="res_aEDate" name="res_aEDate" />
			<input type="text" class="text" id="aEDate" name="aEDate" size="12" disabled="disabled" onchange="setDate_new('mainFrm', 'aEDate');" />
<!--			<input type="hidden" id="aEDate" name="aEDate" value="" />
		...
		<td class="hilite" align="left" valign="top" id="aECal" style="display:table-cell;" nowrap>
			<a href="javascript: void(0);" onclick="switchTypeAA(); return showCalendar('aEDate', '<?php echo $df ?>', 'mainFrm', null, true, true)"
				onFocus="if (document.getElementById('res_aEDate').value == end) switchTypeNull();"
				>
				<img src="<?php echo w2PfindImage('calendar.gif'); ?>" width="24" height="12" alt="<?php echo $AppUI->_('Calendar'); ?>" border="0" />
			</a>
